import express, { type Request, Response, NextFunction } from "express";
import fileUpload from "express-fileupload";
import cron from "node-cron";
import path from "path";
import { registerRoutes } from "./routes";
import { setupVite, serveStatic, log } from "./vite";
import { seedDatabase } from "./seed";
import { generateMonthlyBills, updateOverdueBills } from "./jobs/generateMonthlyBills";
import deliveryRoutes from "./routes/delivery.routes";
import adminDeliveryRoutes from "./routes/admin-delivery.routes";

const app = express();
// Increase body size limit to 50MB to handle large image uploads as base64
app.use(express.json({ limit: '50mb' }));
app.use(express.urlencoded({ extended: false, limit: '50mb' }));
app.use(fileUpload());

// Serve static files from public folder (for banners, etc.)
app.use('/banners', express.static(path.join(process.cwd(), 'public', 'banners')));

app.use((req, res, next) => {
  const start = Date.now();
  const path = req.path;
  let capturedJsonResponse: Record<string, any> | undefined = undefined;

  const originalResJson = res.json;
  res.json = function (bodyJson, ...args) {
    capturedJsonResponse = bodyJson;
    return originalResJson.apply(res, [bodyJson, ...args]);
  };

  res.on("finish", () => {
    const duration = Date.now() - start;
    if (path.startsWith("/api")) {
      let logLine = `${req.method} ${path} ${res.statusCode} in ${duration}ms`;
      if (capturedJsonResponse) {
        logLine += ` :: ${JSON.stringify(capturedJsonResponse)}`;
      }

      if (logLine.length > 80) {
        logLine = logLine.slice(0, 79) + "…";
      }

      log(logLine);
    }
  });

  next();
});

(async () => {
  // Seed database with mock data on first run
  try {
    await seedDatabase();
  } catch (error) {
    log("Error seeding database, continuing anyway...");
  }

  const server = await registerRoutes(app);

  // Register delivery partner routes
  app.use("/api/delivery", deliveryRoutes);
  app.use("/api/admin/delivery-partners", adminDeliveryRoutes);

  // Schedule monthly bill generation - runs at 00:00 on 1st of every month
  cron.schedule("0 0 1 * *", async () => {
    log("🕐 Cron job triggered: Generating monthly bills...");
    await generateMonthlyBills();
  });

  // Schedule overdue bill check - runs daily at 06:00 AM
  cron.schedule("0 6 * * *", async () => {
    log("🕐 Cron job triggered: Checking for overdue bills...");
    await updateOverdueBills();
  });

  log("✅ Cron jobs scheduled: Monthly bills (1st of month) & Overdue check (daily 6 AM)");

  app.use((err: any, _req: Request, res: Response, _next: NextFunction) => {
    const status = err.status || err.statusCode || 500;
    const message = err.message || "Internal Server Error";

    res.status(status).json({ message });
    throw err;
  });

  // importantly only setup vite in development and after
  // setting up all the other routes so the catch-all route
  // doesn't interfere with the other routes
  if (app.get("env") === "development") {
    await setupVite(app, server);
  } else {
    serveStatic(app);
  }

  // ALWAYS serve the app on the port specified in the environment variable PORT
  // Other ports are firewalled. Default to 5000 if not specified.
  // this serves both the API and the client.
  // It is the only port that is not firewalled.
  const port = parseInt(process.env.PORT || '5000', 10);
  server.listen({
    port,
    host: "0.0.0.0",
    reusePort: true,
  }, () => {
    log(`serving on port ${port}`);
  });
})();
